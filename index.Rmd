...
NAP_Fund<-rbind(NAPA_New,GCF_Core,GEF_Grant,readiness_fund)
```
...
```{r}
library(stringr)
GCF_Readiness%>%dplyr::filter(Activity=='Adaptation Planning (AP)')%>%
...
GCF_Readiness %>% dplyr::filter(Activity=='Adaptation Planning (AP)')%>%group_by(Region)%>%count(`Request ID`)%>%
summarise("Projects"=sum(n))%>%
  plot_ly(type = "bar", 
          x =~Projects, 
          y =~Region)
...
GCF_Readiness %>% dplyr::filter(Activity=='Adaptation Planning (AP)')%>% group_by(Category)%>%count(`Request ID`)%>%
summarise("Projects"=sum(n))%>%
  plot_ly(type = "bar", 
          x =~Projects, 
          y=~Category)
